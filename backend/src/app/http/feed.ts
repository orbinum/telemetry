...
export async function feed(c: Context<AppEnv>): Promise<Response> {
  ...
  if (!(await limiters.feed.allow(geo.clientIp(c.req.raw)))) {
    return new Response("too many connection attempts", { status: 429 });
  }

  return registry.chain(genesisHash).fetch(new Request("https://do/feed", c.req.raw));
}
